( function ( global ) {
  ...
  var IMAGES = {
    '2': 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028428251/JnGJqpsxhYNMuiao.png',
    '3': 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028428251/jygiBDNvNUBpCgoC.png',
    '6': 'https://files.manuscdn.com/user_upload_by_module/session_file/310419663028428251/tmquPEOENYIsoUXv.png'
  };
  ...
  // ── Guarantee badge ──────────────────────────────────────────────────────
  // GlycoPulse colors: red #a93226, soft red #e05555
  var BADGE_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 90" width="72" height="72">'
    + '<circle cx="45" cy="45" r="43" fill="#e05555" stroke="#f08080" stroke-width="2"/>'
    + '<circle cx="45" cy="45" r="36" fill="#a93226"/>'
    + '<text x="45" y="30" text-anchor="middle" font-family="Arial" font-size="8" font-weight="bold" fill="#fff">MONEY BACK</text>'
    + '<text x="45" y="48" text-anchor="middle" font-family="Arial" font-size="16" font-weight="bold" fill="#fff">100%</text>'
    + '<text x="45" y="62" text-anchor="middle" font-family="Arial" font-size="7.5" font-weight="bold" fill="#fff">GUARANTEED</text>'
    + '</svg>';
  ...
  // ── CSS — Variant A only (GlycoPulse soft red palette) ───────────────────
  var CSS_A = `
    ...
    .vc-a-card {
      ...
      box-shadow: 0 2px 12px rgba(180,40,40,0.10);
    }
    .vc-a-card:not(.vc-a-best):hover {
      transform: translateY(-6px) scale(1.02);
      box-shadow: 0 16px 40px rgba(224,85,85,0.22), 0 4px 14px rgba(0,0,0,0.07);
      border-color: #e05555;
      text-decoration: none;
      color: inherit;
    }
    .vc-a-card:hover .vc-a-img-wrap img {
      transform: scale(1.10) translateY(-6px);
          filter: drop-shadow(0 0 16px rgba(224,85,85,0.45)) drop-shadow(0 8px 18px rgba(0,0,0,0.14));
    }
    .vc-a-card.vc-a-best {
      border-color: #e05555;
      border-width: 2.5px;
    ...
    .vc-a-badge-top {
      ...
      box-shadow: 0 4px 20px rgba(169,50,38,0.55), 0 0 0 3px #e05555, 0 0 0 5px rgba(224,85,85,0.30);
    ...
    .vc-a-card-headline { font-size: 20px; font-weight: 800; color: #a93226; margin-bottom: 4px; }
    ...
    .vc-a-pill-save { background: linear-gradient(90deg, #e05555, #a93226); color: #fff; font-size: 12px; font-weight: 800; padding: 5px 14px; border-radius: 20px; white-space: nowrap; }
    ...
    @keyframes vc-a-btn-breathe {
      0%, 100% { box-shadow: 0 4px 14px rgba(224,85,85,0.35), 0 0 0 0 rgba(224,85,85,0); }
      50%       { box-shadow: 0 4px 20px rgba(224,85,85,0.55), 0 0 0 5px rgba(224,85,85,0.08); }
    }
    @keyframes vc-a-btn-best-breathe {
      0%, 100% { box-shadow: 0 4px 16px rgba(224,85,85,0.40), 0 0 0 0 rgba(224,85,85,0); }
      50%       { box-shadow: 0 4px 22px rgba(224,85,85,0.65), 0 0 0 5px rgba(224,85,85,0.12); }
    }
    .vc-a-btn {
      display: block;
      width: 100%;
    ...
    .vc-a-total-now { font-weight: 900; color: #a93226; font-size: 18px; }
    ...
    @keyframes vc-a-pulse {
      0%   { box-shadow: 0 12px 40px rgba(224,85,85,0.25), 0 4px 16px rgba(0,0,0,0.08); }
      50%  { box-shadow: 0 16px 50px rgba(224,85,85,0.42), 0 4px 16px rgba(0,0,0,0.08); }
      100% { box-shadow: 0 12px 40px rgba(224,85,85,0.25), 0 4px 16px rgba(0,0,0,0.08); }
    }
  ...
  global.GlycoPulseBump = { render: render, renderA: renderA, init: init, version: VERSION };
